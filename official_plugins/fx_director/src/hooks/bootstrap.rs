...
use plugin_api::{OPPW4_GAME_FLAG_DLC_CHARACTER_SEEN, OPPW4_GAME_FLAG_VIRTUAL_RESOURCE_SEEN};
...
pub(crate) fn install_deferred(api: plugin_api::Oppw4PluginApi, state: SharedFxState) -> i32 {
    ...
    match builder.spawn(move || {
        ...
        if let Some(status) = worker_api.api.game_status() {
            log::write_line(format!(
...
fn wait_for_active_character_gate(api: &WorkerApi, config: FxInstallPlan) {
    ...
    loop {
        if let Some(active) = api.active_character() {
            let has_local_player = config.fx.target != TargetMode::LocalPlayer || active.local_player != 0;
            let matches_character = config.fx.accepts_active_character(active.runtime_id, active.alt_id);
...
fn wait_for_status_gate(api: &WorkerApi, gate: StatusGate) {
    ...
    loop {
        let Some(status) = api.api.game_status() else {
