...
impl FxRuntimeState {
    ...
    pub(super) fn clear_mod_source(&mut self, source: &str) {
        ...
        self.current = self.cycle.preset_at(0).or_else(|| self.effects.last().copied());
...
fn fx_targets_overlap(left: FxConfig, right: FxConfig) -> bool {
    ...
    left.required_character_ids[..left.required_character_id_count as usize]
        .iter()
        .any(|id| {
            right.required_character_ids[..right.required_character_id_count as usize]
                .contains(id)
        })
}
...
pub(crate) fn load_config(api: &Oppw4PluginApi) -> SharedFxState {
    Arc::new(Mutex::new(FxRuntimeState::new(load_plugin_config(api))))
}
