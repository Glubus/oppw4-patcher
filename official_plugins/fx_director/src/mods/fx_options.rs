...
use crate::config::{FxConfig, TargetMode};

pub(super) fn apply_target_from_character(fx: &mut FxConfig, character: &Table) -> mlua::Result<()> {
    if character
...
pub(super) fn fx_from_handle(effect: &Table) -> mlua::Result<FxConfig> {
    ...
    fx.effect_id = effect.get::<Option<u32>>("effect_id")?.unwrap_or(fx.effect_id);
    ...
    fx.loop_end = effect.get::<Option<f32>>("loop_end")?.unwrap_or(fx.loop_end);
    if let Some(ids) = effect.get::<Option<Table>>("required_character_ids")? {
        fx.set_required_character_ids(ids.sequence_values::<u16>().collect::<mlua::Result<Vec<_>>>()?);
    }
    Ok(fx)
}
