...
static RECORD_LOG_COUNT: AtomicU32 = AtomicU32::new(0);
...
pub fn start_service(debug_enabled: bool) {
    ...
    let _ = thread::Builder::new()
        .name("oppw4_active_character".to_string())
        .spawn(|| {
            log::write_line("active character service started");
...
fn install_local_player_hook() {
    ...
    if address == 0 {
        log::write_line("active character hook skipped: signature not found");
        return;
    }
    ...
    match result {
        Ok(()) => log::write_line(format!(
            "active character hook installed site=0x{address:x}"
        )),
        Err(error) => log::write_line(format!(
            "active character hook skipped site=0x{address:x}: {error}"
        )),
    }
}
...
fn poll_active_character() {
    ...
    loop {
        ...
        let active = hooks::active_character_snapshot();
        if active.sequence == 0 || active.sequence == last_sequence {
            continue;
        }
        last_sequence = active.sequence;
        log::write_line(format!(
            "active_character changed runtime={}({}) alt={}({}) local=0x{:x} owner=0x{:x} source=0x{:x} seq={}",
            format_id(active.runtime_id),
            character_label(active.runtime_id),
            format_id(active.alt_id),
            character_label(active.alt_id),
            active.local_player,
            active.fx_owner,
            active.source,
            active.sequence
...
fn character_label(id: u16) -> String {
    if id == u16::MAX {
        return "none".to_string();
    }
    struct_api::characters::find_by_id(id)
