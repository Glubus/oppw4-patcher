...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VirtualReplacementFile {
    pub replacement: VirtualReplacement,
    pub mod_size: u64,
}
...
pub fn build_virtualization_table(
    scan: &ArchiveScan<'_>,
    mod_folder: impl Into<PathBuf>,
) -> Vec<VirtualReplacement> {
    let mod_folder = mod_folder.into();
    let assets: Vec<_> = scan
        .files
        .iter()
        .map(|file| ModAsset {
            file_name: file.file_name.clone(),
            source: ReplacementSource::File(mod_folder.join(&file.file_name)),
        })
        .collect();
    build_virtualization_table_from_assets(scan, &assets)
}
...
fn build_virtual_prefix(block: &crate::RdbBlock) -> Option<Vec<u8>> {
