...
use plugin_api::{
    optional_cstr, HostPluginModVisitorFn, HostPluginModZipVisitorFn, Oppw4ActiveCharacter,
    Oppw4FileProvider, Oppw4GameStatus, Oppw4LogEntry, Oppw4LuaModule, Oppw4PluginApi,
    Oppw4PluginModEntry, OPPW4_PLUGIN_API_VERSION, OPPW4_PLUGIN_MOD_FLAG_ZIP,
};

use super::{debug, logs, lua, mods};
...
unsafe extern "system" fn host_register_lua_module(
...
unsafe extern "system" fn host_register_file_provider(
    ...
    hooks::register_file_provider(provider)
}
...
unsafe extern "system" fn host_game_status(
    ...
    *out_status = hooks::game_status();
    0
}

unsafe extern "system" fn host_active_character(
    _host_context: *mut c_void,
    ...
    *out = hooks::active_character_snapshot();
    0
}

unsafe extern "system" fn host_debug_enabled(_host_context: *mut c_void) -> i32 {
...
unsafe extern "system" fn host_for_each_plugin_mod(
    ...
    for mod_entry in lua_api::discover_mods(&context.plugin_mods_root) {
