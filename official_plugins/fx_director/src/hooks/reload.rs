...
fn cycle_index(base: FxConfig, cycle: CycleConfig, elapsed_ms: u64) -> Option<usize> {
    ...
    match cycle.mode {
        CycleMode::FixedInterval => Some(((elapsed_ms / cycle.interval_ms) % count as u64) as usize),
...
#[cfg(test)]
mod tests {
    use crate::config::{CycleConfig, CycleMode, FxConfig, TargetMode};
