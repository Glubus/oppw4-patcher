...
use crate::mods::{FxInstallPlan, SharedFxState};
...
impl WorkerApi {
    fn new(api: Oppw4PluginApi, state: SharedFxState) -> Self {
        Self {
            api,
            state: Some(state),
        }
    ...
    fn active_character(&self) -> Option<plugin_api::Oppw4ActiveCharacter> {
        self.api.active_character()
    }

    fn debug_enabled(&self) -> bool {
        self.api.debug_enabled()
    }
}
...
fn install_now(worker_api: WorkerApi, config: FxInstallPlan) -> Result<(), String> {
    ...
    if let Some(state) = installer::install_now(&api, worker_api, config)? {
